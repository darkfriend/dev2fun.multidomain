...
$isSaved = CopyDirFiles(
    $pathToModule . '/install/components/dev2fun/multidomain.data',
    $_SERVER["DOCUMENT_ROOT"] . '/bitrix/components/dev2fun/multidomain.data',
    true,
    true
);
if (!$isSaved) {
    throw new Exception('ERRORS_SAVE_FILE '.$_SERVER['DOCUMENT_ROOT'] . 'components/dev2fun/multidomain.data');
}

$isSaved = CopyDirFiles(
    $pathToModule . '/install/components/dev2fun/multidomain.page',
    $_SERVER["DOCUMENT_ROOT"] . '/bitrix/components/dev2fun/multidomain.page',
    true,
    true
);
if (!$isSaved) {
    throw new Exception('ERRORS_SAVE_FILE '.$_SERVER['DOCUMENT_ROOT'] . 'components/dev2fun/multidomain.page');
}
